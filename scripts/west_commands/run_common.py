    ...
    if len(domains) > 1:
        ...
        for d in domains:
            ...
            if cache['BOARD_DIR'] not in processed_boards and 'SOC_FULL_DIR' in cache:
                ...
                for grp in data_yaml['runners']['run_once']:
                    for cmd, data in grp.items():
                        for group in data['groups']:
                            run_first = bool(data['run'] == 'first')
                            used_cmds.append(UsedFlashCommand(cmd, group[group_type], data['runners'], run_first))
        ...
        for l, match in enumerate(entry.boards):
            ...
            if match[0] == '/' and match[-1] == '/':
                ...
                for check in board_names:
                    if re.match(fr'^{match[1:-1]}$', check) is not None:
                        match_found = True
                        break
            elif match in board_names:
                match_found = True
            ...
            for cmd in used_cmds:
                if cmd.command == runner_args[i] and (runner_name in cmd.runners or 'all' in cmd.runners):
                    ...
                    for match in cmd.boards:
                        if match[0] == '/' and match[-1] == '/' and re.match(fr'^{match[1:-1]}$', board) is not None:
                            # Check if there is a matching board for this regex
                            match_found = True
                            break
                        elif match == board:
                        ...
                        for check in cmd.boards:
                            ...
                            if check[0] == '/' and check[-1] == '/':
                                for match in board_image_count:
                                    if re.match(fr'^{check[1:-1]}$', match) is not None:
                                        if board_image_count[match].flashed == board_image_count[match].total:
                                            can_continue = True
                                            break
                            elif board_image_count[check].flashed == board_image_count[check].total:
                                can_continue = True
