...
#define DT_NODE_BY_MAPPED_PARTITION_LABEL(label)				\
	DT_CAT3(DT_COMPAT_zephyr_mapped_partition_LABEL_, label, _EXISTS)

/**
 * @brief Test if a mapped partition with a given label property exists
 * @param label lowercase-and-underscores label property value
 * @return 1 if the device has a "zephyr,mapped-partition" compatible,
 *         0 otherwise.
 */
 ...
 */
#define DT_MEM_FROM_MAPPED_PARTITION2(node_id)						\
	COND_CODE_1(DT_NODE_HAS_COMPAT(DT_CAT(node_id, _NVM_DEVICE), soc_nv_flash),	\
				       (DT_CAT(node_id, _NVM_DEVICE)),			\
				       (DT_INVALID_NODE))

#define DT_MEM_FROM_MAPPED_PARTITION(node_id) DT_CAT(node_id, _NVM_DEVICE)
...
#define DT_MTD_FROM_MAPPED_PARTITION(node_id)					\
	DT_PHANDLE(DT_MEM_FROM_MAPPED_PARTITION(node_id), flash_peripheral_dev)
