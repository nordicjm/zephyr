 ...
 * read/write/save/commit request and to authorise or deny it. Access will be allowed so long as
 * no handlers return an error, if one returns an error then access will be denied.
 */
struct settings_mgmt_access {
	 ...
	 * SETTINGS_ACCESS_DELETE). Note that this can be changed by handlers to
	 * redirect settings access if needed (as long as it does not exceed the maximum setting
	 * string size) if CONFIG_MCUMGR_GRP_SETTINGS_BUFFER_TYPE_STACK is selected.
	 */
