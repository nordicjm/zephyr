...
#ifdef CONFIG_MCUMGR_GRP_TRANSPORT
static bool smp_bt_bridge_connect(struct smp_transport_bridge *bridge, bool outgoing, bool same_transport, zcbor_state_t *input_data, zcbor_state_t *output_data)
{
...
static int smp_bt_bridge_details(zcbor_state_t *output_data)
{
	bool ok;
...
static int smp_bt_bridge_config_details(uint32_t type, zcbor_state_t *output_data)
{
//TODO
	return MGMT_ERR_EOK;
}
...
#if defined(CONFIG_MCUMGR_GRP_TRANSPORT_INFO_FUNCTIONS)
	smp_bt_transport.functions.bridge_details = smp_bt_bridge_details;
