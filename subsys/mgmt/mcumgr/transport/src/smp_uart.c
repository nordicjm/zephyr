...
static bool smp_uart_bridge_connect(struct smp_transport_bridge *bridge, bool outgoing, bool same_transport, zcbor_state_t *input_data, zcbor_state_t *output_data)
{
return true;
}
...
static int smp_uart_bridge_modes(zcbor_state_t *output_data)
{
	bool ok;
...
static int smp_uart_bridge_config_details(uint32_t type, zcbor_state_t *output_data)
{
	return MGMT_ERR_EOK;
}
#endif
