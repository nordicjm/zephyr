...
static int smp_raw_dummy_bridge_tx(const struct smp_transport_bridge *bridge, struct net_buf *nb, bool outgoing)
{
	return smp_raw_dummy_tx_pkt_int(nb);
}

#if defined(CONFIG_MCUMGR_GRP_TRANSPORT_INFO_FUNCTIONS)
static int smp_raw_dummy_bridge_details(zcbor_state_t *output_data)
{
	bool ok;
...
static int smp_raw_dummy_bridge_config_details(uint32_t type, zcbor_state_t *output_data)
{
	return MGMT_ERR_EOK;
}
#endif
...
#if defined(CONFIG_MCUMGR_GRP_TRANSPORT_INFO_FUNCTIONS)
	smp_dummy_transport.functions.bridge_details = smp_raw_dummy_bridge_details;
