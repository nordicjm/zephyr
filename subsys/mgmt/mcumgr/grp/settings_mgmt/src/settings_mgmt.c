...
static int settings_mgmt_translate_error_code(uint16_t ret)
{
	...
	switch (ret) {
	case SETTINGS_MGMT_RET_RC_KEY_TOO_LONG:
	rc = MGMT_ERR_EINVAL;
	break;
	case SETTINGS_MGMT_RET_RC_KEY_NOT_FOUND:
	case SETTINGS_MGMT_RET_RC_READ_NOT_SUPPORTED:
	rc = MGMT_ERR_ENOENT;
	break;
	case SETTINGS_MGMT_RET_RC_UNKNOWN:
	default:
