	...
	if (bridge_active == true) {
		...
		while (i < CONFIG_MCUMGR_GRP_TRANSPORT_MAX_BRIDGES) {
			if (bridges[i].status == 1 && ((bridges[i].incoming_transport == transport
			    && outgoing == false) || (bridges[i].outgoing_transport == transport
			    && outgoing == true))) {
				bridged = true;
				break;
			}
...
int transport_mgmt_disconnect_transport(struct smp_transport *transport, bool incoming,
					bool outgoing)
{
	struct smp_transport_bridge *bridge = NULL;
	...
	if (bridge_active == true) {
		...
		while (i < CONFIG_MCUMGR_GRP_TRANSPORT_MAX_BRIDGES) {
			if (bridges[i].status == 1 && ((bridges[i].incoming_transport == transport
			    && incoming == true) || (bridges[i].outgoing_transport == transport
			    && outgoing == true))) {
				bridge = &bridges[i];
				break;
			}
	...
	if (bridge != NULL) {
		return transport_mgmt_disconnect_bridge(bridge);
	}
