...
#define OS_MGMT_HANDLERS 1
#define ENUM_MGMT_HANDLERS 3
...
static bool parse_details_entries(zcbor_state_t *state, void *user_data)
{
	...
	while (!zcbor_array_at_end(state)) {
		...
		if (group == MGMT_GROUP_ID_ENUM) {
			...
			if (strcmp(name.value, "enum mgmt") == 0) {
		...
		if (group == MGMT_GROUP_ID_OS) {
			...
			if (strcmp(name.value, "os mgmt") == 0) {
				entry_data[index].matched_name = true;
			}
		}
#endif
#if defined(CONFIG_MCUMGR_GRP_SHELL)
		if (group == MGMT_GROUP_ID_SHELL) {
			index = FOUND_INDEX_SHELL;
		}
#endif
		else {
...
ZTEST(enum_mgmt, test_details)
{
	...
	/* Get the response value to compare */
LOG_HEXDUMP_ERR(nb->data, nb->len, "ww");
	...
	while (i < TEST_GROUPS) {
		zassert_true(details_response[i].matched_name, "Expected group name to be found in details");
		zassert_true(details_response[i].matched_handlers, "Expected group handler to be found in details");
		zassert_true(details_response[i].matched_test, "Did not expect group test to be found in details");
		++i;
	}
...
ZTEST(enum_mgmt, test_details_extra)
{
	...
	while (i < TEST_GROUPS) {
		zassert_true(details_response[i].matched_name, "Expected group name to be found in details");
		zassert_true(details_response[i].matched_handlers, "Expected group handler to be found in details");
		zassert_true(details_response[i].matched_test, "Expected group test to be found in details");
		++i;
	}
