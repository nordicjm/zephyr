...
Z_TEST_SKIP_IFNDEF(CONFIG_SETTINGS_SAVE_SINGLE_SUBTREE_WITHOUT_MODIFICATION);
	...
	zassert_false(access_invalid_got, "Did not expect an invalid access notification type");
	zassert_false(event_invalid_got, "Did not expect an invalid event");
	zassert_str_equal(access_name, "first/expected_fail/value4", "Expected notification name to be correct");
	...
	zassert_equal(group_error.group, MGMT_GROUP_ID_SETTINGS, "Expected group to be settings");
	zassert_equal(group_error.rc, SETTINGS_MGMT_ERR_SAVE_NOT_SUPPORTED,
