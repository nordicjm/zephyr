...
{
	(void)cb("first/expected_fail/value4", &single_data.forth_val, sizeof(single_data.forth_val));
...
ZTEST(settings_functional, test_single_save)
{
	...
	rc = settings_save_subtree_or_single_without_modification("first/other/value1", true, false);
	zassert_true(rc == -EPERM);
	zassert_equal(single_data.first_val, 0x08);
	zassert_equal(single_data.second_val, 0x09);
	zassert_equal(single_data.third_val, 0x0a);
	...
	single_data.forth_val = 0x44;
	rc = settings_save_subtree_or_single_without_modification("first/expected_fail/value4", false, true);
